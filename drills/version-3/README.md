...
1. Using the included `index.html` file, create a form that includes:
    ...
    * An image with the placeholder graphic `assets/placeholder.jpg` and the class "role-preview"
    * A submit button with the class "save"
...
1. Using the included `app.js` file, add the following behavior
    ...
    * When the result of the `POST` from the save button returns, you should display the resulting `message` should be added to the `.save-status` paragraph, displayed for 2000ms, and then removed.
